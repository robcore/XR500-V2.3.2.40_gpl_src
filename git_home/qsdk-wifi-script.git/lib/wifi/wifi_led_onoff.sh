...
on_led_qcawifi () {
    ...
    local radio_type=$3
    local led_option=$4
    local wlg_guest_type=$5
    local wla_guest_type=$6
    ...
    WIFI_GUEST_LED_ON='ledcontrol -n guestwifi -c green -s on'
    WIFI_GUEST_LED_OFF='ledcontrol -n guestwifi -c green -s off'
    ...
    if [ "$radio_num" -gt "0" ]; then
        eval ${WIFI_LED_ON}
        [ "$wlg_guest_type" -gt "0" -o "$wla_guest_type" -gt "0" ] && eval ${WIFI_GUEST_LED_ON} || eval ${WIFI_GUEST_LED_OFF}
    else
        eval ${WIFI_LED_OFF}
    fi
