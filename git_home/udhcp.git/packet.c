		   ...
		   u_int32_t dest_ip, int dest_port, unsigned char *dest_arp, int ifindex)
{
	...
	/*
	check option in section 9.10 of RFC-2132, it says,
	...
	The IP Total Length and UDP Length must be large enough to  contain the minimal BOOTP
	header of 300 octets (in the UDP data field) specified in [1]
	...
	*/
	if (p_len < 300) p_len = 300;
		   ...
		   u_int32_t dest_ip, int dest_port)
{
	...
	...
	*/
	int len = udhcp_get_payload_len(payload);
	if (len < 300) len = 300;

	result = write(fd, payload, len);
#else
