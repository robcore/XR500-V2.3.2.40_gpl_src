...
#define LOPT_TRY_ALL_NS    350

#ifdef DNI_PARENTAL_CTL
#define LOPT_PARENTAL_CONTROL  901
#endif

#ifdef SUP_STATIC_PPTP
#define LOPT_STATIC_PPTP       902
#endif
...
#ifdef DNI_PARENTAL_CTL
	{"parental-control", 2, 0, LOPT_PARENTAL_CONTROL},
#endif

#ifdef SUP_STATIC_PPTP
	{"static-pptp", 2, 0, LOPT_STATIC_PPTP},
#endif

#ifdef BIND_SRVSOCK_TO_WAN
	{"wan-interface", 1, 0, LOPT_WAN_INTERFACE},
#endif
...
#ifdef DNI_PARENTAL_CTL
  { LOPT_PARENTAL_CONTROL, ARG_ONE, "    --parental-control[=<file>]", gettext_noop("Enable Parental Control and specify deviceid file(default to /tmp/parentalcontrol.conf)."), NULL },
#endif

#ifdef SUP_STATIC_PPTP
  { LOPT_STATIC_PPTP, ARG_ONE, "    --static-pptp[=<file>]", gettext_noop("Support static pptp for Russian Version(file default to /tmp/pptp.conf)."), NULL },
#endif

#ifdef BIND_SRVSOCK_TO_WAN
  { LOPT_WAN_INTERFACE, ARG_ONE, "    --wan-interface=name", gettext_noop("interface name of wan, such as \"eth0\" or \"ppp0\"."), NULL },
#endif
...
#  endif
	...
	*cnt = 0;
...
#ifdef DNI_PARENTAL_CTL
	case LOPT_PARENTAL_CONTROL:
	  {
		  parentalcontrol_enable = 1;
		  if (arg) {
			  pc_table_file = opt_string_alloc(arg);
		  }
		  break;
	  }
#endif
...
#ifdef SUP_STATIC_PPTP
	case LOPT_STATIC_PPTP:
	  {
		  if (arg) {
			  daemon->static_pptp_conf = opt_string_alloc(arg);
		  } else {
			  daemon->static_pptp_conf = DEF_STATIC_PPTP_CONFIG;
		  }
		  if (0 == load_static_pptp_config(daemon))
			  daemon->static_pptp_enable = 1;
		  break;
	  }
#endif

#ifdef BIND_SRVSOCK_TO_WAN
	case LOPT_WAN_INTERFACE:
	  {
		  wan_ifname = opt_string_alloc(arg);
		  break;
	  }
#endif
    ...
    case LOPT_TRY_ALL_NS: /* --continue try next nameserver 
							 ...
							 response code 
							 */
      set_option_bool(OPT_TRY_ALL_NS);
	break;
...
#ifdef SUP_STATIC_PPTP
  daemon->static_pptp_enable = 0;
  daemon->record_static_route = 0;
  daemon->pptp_resolv_file = NULL;
#endif
