...
static struct listener *create_raw_ipv6_listener(union mysockaddr *addr, int do_tftp, int dienow)
{
...
#  endif
  ...
  if(config_match("ap_mode", "1")) {
	ll6 = create_raw_ipv6_listener(&addr, !!option_bool(OPT_TFTP), 1);
	l6->next = ll6;
  }
#endif
