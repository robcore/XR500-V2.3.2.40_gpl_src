...
static unsigned long ntransmitted, nreceived, nrepeats, pingcount, timeout;
static int myid;
...
static void sendping(int junk)
{
	...
	if (pingcount == 0 || ntransmitted < pingcount) {	/* schedule next in 1s */
		...
		alarm( timeout > 0 ? timeout : MAXWAIT );
	}
}
...
static void ping(const char *host)
{
	...
	printf(": %d data bytes\n", datalen);
	fp = fopen("/tmp/ping_reso", "w");
        if ( fp != NULL ) {
		snprintf(resolv, 256, "Resoved data %s\n", hostent->h_name);
		fputs(resolv, fp);
                fclose(fp);
        }
...
make it true for ping. */
        ...
        if( option_mask32 & 16 ) {  // -W
          timeout = xatoul(opt_W);
        }
