...
static inline unsigned int
hash_by_dst(const struct net *net, const struct nf_conntrack_tuple *tuple)
{
	unsigned int hash;
		  ...
		  enum nf_nat_manip_type maniptype)
{
	...
	*/
	...
	if (have_to_hash_modified) {
		...
		dsthash = hash_by_dst(net, ct->status & IPS_SNATP2P_SRC
				? &ct->tuplehash[IP_CT_DIR_REPLY].tuple
				: &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
		spin_lock_bh(&nf_nat_lock);
		nat = nfct_nat(ct);
...
static void nf_nat_cleanup_conntrack(struct nf_conn *ct)
{
	...
	if (sysctl_snatp2p_range_port_full_control &&
	    !(nat->ct->status & IPS_RANGE_PORT_FULL)) {
		unsigned int dport;
...
#if defined(CONFIG_IP_NF_TARGET_SNATP2P) || defined(CONFIG_IP_NF_TARGET_SNATP2P_MODULE) || defined(CONFIG_IP_NF_TARGET_HAIRPIN) || defined(CONFIG_IP_NF_TARGET_HAIRPIN_MODULE)
	nat_htable_size = net->ipv4.nat_htable_size;
	net->ipv4.nat_by_modified_source = nf_ct_alloc_hashtable(&net->ipv4.nat_htable_size, 0);
	BUG_ON(nat_htable_size != net->ipv4.nat_htable_size);
...
#if defined(CONFIG_IP_NF_TARGET_SNATP2P) || defined(CONFIG_IP_NF_TARGET_SNATP2P_MODULE) || defined(CONFIG_IP_NF_TARGET_HAIRPIN) || defined(CONFIG_IP_NF_TARGET_HAIRPIN_MODULE)
	nf_ct_free_hashtable(net->ipv4.nat_by_modified_source, net->ipv4.nat_htable_size);
#endif
}
