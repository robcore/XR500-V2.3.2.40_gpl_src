...
{
	var sub_list = "setup,usb,security,green,admin,advanced,moniter";
	...
	for ( i=0; i< sub_array.length; i++)
	{
		...
		{
			var div_name = document.getElementById(button_name);
			if( !div_name)
				continue;
...
function open_sub(name)
{
	...
	var open_flag= document.getElementById(sub_name).style.display;	
	if( open_flag == "none")
	{
...
function open_or_close_sub(name)
{
	...
	var button_div = document.getElementById(button_name);
	if( open_flag == "none")
	{
		settingClass(button_div, "advanced_white_open_button");
		document.getElementById(sub_name).style.display="";
...
function change_menu_height()
{
	...
	if(menu_height > 510)
	{
		document.getElementById("middle").style.minHeight = is_double ? (menu_height + 87)+"px" : (menu_height+ 45)+"px";
		if(isIE() == true)
			document.getElementById("middle").style.height = (con_height+94)+"px";
	}
	else
	{
		if(isIE() == true)
		{
			...
			document.getElementById("middle").style.height = is_double ? (menu_height + 87)+"px" : (con_height + 94)+"px";
		}
		else
			document.getElementById("middle").style.minHeight = is_double ? (menu_height + 87)+"px" : (con_height - 5)+"px";
	}
...
var devmode_items = [ "internet", "adv_setup_wizard", "adv_wps", "wireless", "guest", "wan", "lan", "qos", "readyshare_stor", "usb_bas", "usb_adv", "media", "print", "vault", "block_site", "block_services", "schedule", "email", "wladv", "rae", "forwarding", "triggering", "dns", "vpn", "static", "remote", "upnp", "traffic", "wl_bridge", "ipv6", "parental", "access_control", "usb_set", "vlan", "led", "devname" ];
function enabledItemsClass()
{
	...
	{
		var item_group;
		enable_flag = eval(""+enable_flags[i]);
		item_group = eval(array_name[i]);
		for(j=0; j<item_group.length; j++)
		{
			var cur_div = document.getElementById(item_group[j]);
	...
	if(have_lte_flag ==1 )
	{
		var cur_div = document.getElementById("mobile_internet");
		var cur_div_1= document.getElementById("internet");
		var height = cur_div.getElementsByTagName("a")[0].clientHeight;
...
function menu_class_default()
{
    ...
    var extensible_items = ["setup_bt", "usb_bt", "security_bt", "green_bt", "admin_bt", "advanced_bt", "moniter_bt"];
	var i;
	
	
	//first page open internet. when old_div is blank, change class to default
	if( old_div ===  undefined || old_div == "")
	{
		menu_div = document.getElementById("internet");
		menu_div.className = "middle_name";
	}
...
function menu_color_change( change_id )
{
	...
	var current_div = document.getElementById(change_id);
	
	if( change_id == "adv_home")
	{
	...
	{
		var parent_id = document.getElementById(change_id).parentNode.parentNode.id;
                var btn_id = parent_id.replace('sub', 'bt');
                var btn_div = document.getElementById(btn_id);
...
function click_adv_action(id)
{
	if(id == "adv_setup_wizard")
		...
		{
			var item_group;
			var enable_flag = eval(enable_flags[i]);
		...
		if( id == "adv_home" )
			...
			if(enable_bridge_flag == "1")
                ...
                else if( id == "adv_setup_wizard" && master == "admin" && enable_ap_flag != "1" && enabled_wds != "1" && enable_bridge_flag != "1" && enable_extender_flag != "1" && device_mode != "1")
                {
			menu_color_change('adv_setup_wizard');
			if(dsl_enable_flag == "1")
				goto_formframe('DSL_WIZ_sel.htm');
			else
			   if (((netgear_region.toUpperCase() == "WW" || netgear_region == "") && gui_region == "Russian" ) || netgear_region.toUpperCase() == "RU")
				goto_formframe('RU_WIZ_sel.htm');
			  else
				goto_formframe('WIZ_sel.htm');
		}
                
		else if( id == "adv_wps" && enabled_wps == "1" && enabled_wds != "1" && enable_bridge_flag != "1" && device_mode != "1" )
                {
			...
			menu_color_change('adv_wps');
                ...
                {
			if(!(have_lte_flag==1 && broadband_wan_type == "MyDetc"))
			{
		...
		else if( id == "mobile" )
		{
			...
			menu_color_change('block_services');
                ...
                else if( id == "schedule")//&& enable_ap_flag != 1
                {
		...
		else if( id == "ipmac")
		{
			...
			menu_color_change('wladv');
                ...
                else if( id == "wds")//&& enable_ap_flag != 1 for bug 30286 moved here
                {
			goto_formframe("WLG_wds.htm");
			menu_color_change('wds');
                }
                else if( id == "forwarding" )
                {
					goto_formframe("forwarding.htm");
					menu_color_change('forwarding');
                }
                else if( id == "triggering" )
                {
					goto_formframe("triggering.htm");
					menu_color_change('triggering');
                }				
                else if( id == "dns" )
                {
		...
		else if( id == "lte_broadband" && have_lte_flag==1)
		{
			goto_formframe("BAS_lte_broadband.htm");
			menu_color_change(id);
		}
		else if( id == "mobile_internet" && broadband_wan_type != "AutoDetc")
		{
			click_lte_sim=0;
		...
		{
			goto_formframe("RST_statistic.htm");
			menu_color_change('statistics');			
			
		}
		else if( id == "reboot")
		{
			goto_formframe("FW_reboot.htm");
			menu_color_change(id);
			
 		}
		else if( id == "conn_status" )
		{
				goto_formframe("RST_conn_status.htm");
