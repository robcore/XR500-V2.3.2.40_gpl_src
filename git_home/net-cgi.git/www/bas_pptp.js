...
function isValidserv_IP(ip)
{
	...
	{
		if(parseInt(ss[i])<0 || parseInt(ss[i])>255)
		{
			return false;
		...
		if(i==0){
			if(parseInt(ss[i])=="0"||parseInt(ss[i])=="127"||parseInt(ss[i])=="224")
					return false;
			}
		if(i==3){
			if(parseInt(ss[i])=="0")
	...
	if(parseInt(ss[0])=="255"&&parseInt(ss[1])=="255"&&parseInt(ss[2])=="255"&&parseInt(ss[3])=="255")
		return false;
	return true;
}
