...
#include <asm/errno.h>
#include "hw29764958p0p256p512p4x4p4x4pcascade_board_param.h"
#include "hw29764958p0p256p512p4x4p4x4pcascade.h"
...
int checkboard(void)
{
	printf("U-boot 2012.07 dni1 V0.2 for DNI HW ID: 29764958 NOR flash 0MB; NAND flash 256MB; RAM 512MB; 1st Radio 4x4; 2nd Radio 4x4; Cascade\n");
	return 0;
}
...
void board_upgrade_string_table(unsigned char *load_addr, int table_number, unsigned int file_size)
{
    ...
    ret = nand_read_skip_bad(
            &nand_info[0], CONFIG_SYS_STRING_TABLE_ADDR_BEGIN, &read_size,
    ...
    update_data(string_table_addr, CONFIG_SYS_STRING_TABLE_TOTAL_LEN,
...
int do_upgrade_uboot (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
       ...
       snprintf(runcmd, sizeof(runcmd), "setenv machid 136c");
       ...
       run_command(runcmd, 0);
       snprintf(runcmd, sizeof(runcmd), "ipq_nand linux && nand erase 0x01180000 0x00080000 && imgaddr=0x42000000 && source $imgaddr:script");
       run_command(runcmd, 0);
       return 0;
}
...
{
       char runcmd[256];
       snprintf(runcmd, sizeof(runcmd), "nand erase 0x1180000 0x80000");
       run_command(runcmd, 0);
       return 0;
}
...
{
       printf("XR500 U-boot dni1 V0.2 for DNI HW ID: 29764958; NAND flash 256MB; RAM 512MB CASCADE .\n");
       return 0;
}
