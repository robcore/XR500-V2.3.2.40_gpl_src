...
#include <asm/hw29765352p32p0p512p2x2p2x2p4x4/smem.h>
#include <mmc.h>
#include "hw29765352p32p0p512p2x2p2x2p4x4.h"
...
static int inline do_dumpipq_data(void)
{
	...
	if (run_command("dumpipq_data", 0) != CMD_RET_SUCCESS) {
		...
		"\nPress any key to take control of U-Boot");
...
static int do_boot_unsignedimg2(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
#ifdef CONFIG_QCA_APPSBL_DLOAD
	uint64_t etime;
	volatile u32 val;
...
if (atoi(board_data) == 0x1){
	...
	if (sfi->flash_type == SMEM_BOOT_NAND_FLASH) {
		...
		if (sfi->rootfs.offset == 0xBAD0FF5E) {
			 ...
			 * Kernel is in a separate partition
			 */
			uint kernel_size = 0x400000;
			snprintf(runcmd, sizeof(runcmd),
				"sf probe &&"
				"sf read 0x%x 0x%x 0x%x && ",
				CONFIG_SYS_LOAD_ADDR, (uint)(CFG_IMAGE_BASE_ADDR), (uint)CONFIG_SYS_FLASH_SECTOR_SIZE);
			...
			uint header = * (uint*) ( CONFIG_SYS_LOAD_ADDR );
			if( ntohl(header) == 0xd00dfeed ) {
				kernel_size = ntohl(* (int*) (CONFIG_SYS_LOAD_ADDR + 4));
				printf("kernel size : %x\n", kernel_size);
			}
			snprintf(runcmd, sizeof(runcmd),
				"sf probe &&"
				"sf read 0x%x 0x%x 0x%x && ",
				CONFIG_SYS_LOAD_ADDR, (uint)(CFG_IMAGE_BASE_ADDR), (uint) kernel_size);
		}
#ifdef CONFIG_QCA_MMC
	} else if (sfi->flash_type == SMEM_BOOT_MMC_FLASH) {
		if (debug) {
...
static int do_bootipq2(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (ret == 0 || ret == -EOPNOTSUPP) {
		return do_boot_unsignedimg2(cmdtp, flag, argc, argv);
	}
...
static int do_boot_check_crashdump(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
#ifdef CONFIG_QCA_APPSBL_DLOAD
	uint64_t etime;
	volatile u32 val;
	...
	u8 board_data[BOARD_DATA_LENGTH + 1];
	get_board_data_in_art(board_data);
	printf("crashdump switch : %x\n", atoi(board_data));
if (atoi(board_data) != 0x1){
	printf("crashdump switch is off, tftp-crashdump is disable\n");
	return CMD_RET_SUCCESS ;
}
	printf("crashdump switch is on, tftp-crashdump is enable\n");
#ifdef CONFIG_QCA_APPSBL_DLOAD
	ret = scm_call(SCM_SVC_BOOT, SCM_SVC_RD, NULL,
			0, (void *)&val, sizeof(val));
	/* check if we are in download mode */
...
static int do_bootcheck(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	int ret;
	char buf;
	/*
	...
	if (ret == 0 || ret == -EOPNOTSUPP) {
		return do_boot_check_crashdump(cmdtp, flag, argc, argv);
	}

	return CMD_RET_SUCCESS;
}

U_BOOT_CMD(bootcheck, 2, 0, do_bootcheck,
	   "boot check crashdump",
	   "boot [debug] - Check crashdump\n");



int do_crashdump_switch_set(cmd_tbl_t *cmdtp, int flag, int argc,
                            char * const argv[])
{
	if (argc ==2 && (atoi(argv[1])==1 || atoi(argv[1])==0)){
		printf("argc is %d , argv[1] is %d \n",argc ,atoi(argv[1]));
		do_board_data_set(cmdtp, flag, argc,argv);
	}else
		printf("usage: crashdump_switch_set 1/0 to on/off switch\n");
	return 0;
}

U_BOOT_CMD(
	crashdump_switch_set, 2, 0, do_crashdump_switch_set,
	"set 1/0 to enable/disable crashdump_switch on board",
	" For instance: crashdump_switch_set 1"
);


int do_crashdump_switch_show(cmd_tbl_t *cmdtp, int flag, int argc,
                             char * const argv[])
{
	u8 board_data[BOARD_DATA_LENGTH + 1];
	get_board_data_in_art(board_data);
	if(atoi(board_data) == 0x1)
		printf("crashdump_switch value is 0x1 , tftp-crashdump is enabled\n");
	else
		printf("crashdump_switch value is %x , tftp-crashdump is disabled\n",atoi(board_data));
	return 0;
}

U_BOOT_CMD(
	crashdump_switch_show, 1, 0, do_crashdump_switch_show,
	"Show current crashdump_switch",
	"\n"
	" For instance: crashdump_switch"
);
