...
#ifdef DNI_NAND
	ret = nand_read_skip_bad(&nand_info[0], BOARDDATA2_ADDR, &read_size, (u_char *)buffer);
	printf(" %zu bytes read: %s\n", read_size,
	       ret ? "ERROR" : "OK");
#endif
...
#ifdef DNI_NAND
	ret = nand_read_skip_bad(&nand_info[0], BOARDDATA2_ADDR, &read_size, (u_char *)sectorBuff);
	printf(" %zu bytes read: %s\n", read_size,
	       ret ? "ERROR" : "OK");
#else
#ifdef CONFIG_FUNC_MMC
