...
char * uh_file_unix2date(time_t ts)
{
	static char str[128];
...
void uh_file_request(struct client *cl, struct http_request *req, struct path_info *pi)
{
	...
	if( (pi->stat.st_mode & S_IFREG) && ((fd = open(pi->phys, O_RDONLY)) > 0) )
	{
		...
		if( ok > 0 )
		{
			...
			ensure_out(uh_http_sendf(cl, NULL, "Content-Length: %u\r\n", (unsigned int )pi->stat.st_size));
			ensure_out(uh_http_sendf(cl, NULL, "Cache-Control: max-age=%u\r\n", 31536000 ));
