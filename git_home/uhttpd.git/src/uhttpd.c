...
static void uh_dispatch_dumaos_static_request(
	struct client *cl, struct http_request *req, struct path_info *pin, struct config *conf)
{
	int i;
	static const char* ND_i18n_extensions[] = {
		".html",
		".htm",
		".js",
		"poll.json",
...
static void uh_dispatch_request(
	struct client *cl, struct http_request *req, struct path_info *pin, struct config *conf)
 {
	...
	static const char* ND_static_dirs[] = {
		"/nd-js/",
		"/desktop/",
		"/json/",
		"/libs/",
	...
	/* NETDUMA, implement standard behaviour otherwise DNI method */
	if( conf->disable_autocgi ){
		if( uh_path_match(cl->server->conf->cgi_prefix, pin->name)
				|| (ipr = uh_interpreter_lookup(pin->phys) ) ){
			uh_cgi_request(cl, req, pin, ipr, conf);
		} else {
			uh_dispatch_dumaos_static_request( cl, req, pin, conf );
		}
	} else {	/* try cgi everything apparently ?! */

	//	if( !uh_path_match( "/desktop/index.html", pin->name ) &&
	//					!uh_path_match("/desktop/app-index.html", pin->name ) ){
			for( i = 0; i < ( sizeof( ND_static_dirs ) / sizeof( char* ) ); i++ ){
				if( uh_path_match( ND_static_dirs[i], pin->name ) ){
					uh_dispatch_dumaos_static_request( cl, req, pin, conf );
					return;
				}
			}
...
/* cleanup client */
static void uh_cleanup_client( int cur_fd ){
	/* close client socket */
	close(cur_fd);
...
/* process client in own thread */
static void uh_process_client( struct config *conf, struct client *cl, int cur_fd ){
	/* working structs */
	...
	if( (req = uh_http_header_recv(cl)) != NULL )
	{
				...
				!sa_rfc1918(&cl->servaddr) )
		{
					...
					uh_path_match(conf->lua_prefix, req->url) )
			{
				...
				pin = uh_path_lookup(cl, req->url);
				if( uh_cgi_auth_check(cl, req, NULL, 1, pin ? 0 : 1 ) ){
					if( pin )
						uh_dispatch_dumaos_static_request(cl, req, pin, conf);
					else
				...
				{
					/* auth ok? */
					if( !pin->redirected && uh_auth_check(cl, req, pin)&& uh_cgi_auth_check(cl, req, pin, 0, 0))
		...
		/* 404 */
				else
				{
					...
					if( pin && uh_auth_check(cl, req, pin) && uh_cgi_auth_check(cl, req, pin, 0, 0))
					{
				...
				else
				{
					...
					if( ! (cl = uh_client_lookup(cur_fd)) )
					{
						...
						switch( fork() ){
							...
							case 0:
								close_all_listeners();
								uh_process_client( conf, cl, cur_fd );
								exit(0);
								break;
							default:
								if(conf->tls)
...
void fw_checking()
{
//	fprintf(stderr, "AUTO FW CHECK server start\n");
...
//	fprintf(stderr, "week_day == %d\n", week_day);
	...
	{/* check current time every half hour. */
		...
		setenv("TZ", p, 1);
		tm = localtime(&now);
//		fprintf(stderr, "tm->tm_wday == %d, hour=%ld, min=%ld ,sec=%ld\n", tm->tm_wday, (long)tm->tm_hour, (long)tm->tm_min, (long)tm->tm_sec);
...
int main (int argc, char **argv)
{
	...
	while( (opt = getopt(argc, argv,
		"fSDRC:K:E:I:p:s:h:c:l:L:d:r:m:x:i:t:T:A:u")) > 0
	) {
		switch(opt)
		{
			/* NETDUMA, implement standard behaviour otherwise DNI method */
			case 'u':
#ifdef HAVE_CGI
				conf.disable_autocgi = 1;
				fprintf(stderr,"DISABLE CGI BABES\n");
#endif
				break;
			/* [addr:]port */
			case 'p':
