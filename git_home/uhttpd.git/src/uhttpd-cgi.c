...
void do_uh_cgi_request(
	...
	switch( (child = fork()) )
	{
		...
		case 0:
			...
			if( !strstr( pi->phys, cl->server->conf->cgi_prefix ) && binary == NULL )
...
void uh_cgi_request(
	...
	struct path_info *pi, struct interpreter *ip
	,struct config *conf) {
	do_uh_cgi_request( cl, req, pi, ip, conf, NULL, NULL );
}
