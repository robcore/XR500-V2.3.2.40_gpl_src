...
connect_server()
{
	...
	while ! ls ${ovpn_client_cfg_dir}/${hidemyass}/${proto}/${1}.${2}*.${proto}.ovpn >/dev/null 2>&1
	do
		sleep 10
	done
	...
	for cfg_file in $prior_cfg_file ${ovpn_client_cfg_dir}/${hidemyass}/${proto}/${1}.${2}*.${proto}.ovpn
	do
