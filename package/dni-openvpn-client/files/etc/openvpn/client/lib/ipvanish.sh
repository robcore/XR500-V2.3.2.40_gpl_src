...
bin=${0##*/}
opvncfg_dir="${ovpn_client_cfg_dir}/${ipvanish}"
ca_file="${opvncfg_dir}/ca.ipvanish.com.crt"
...
connect_server()
{
	...
	[ "${proto}" = "TCP" ] && proto="tcp-client" || proto="udp"

	# wait for sync configs from netgear site
	while ! ls ${ovpn_client_cfg_dir}/${ipvanish}/ipvanish-${1}-${2}*.ovpn >/dev/null 2>&1
	do
		sleep 10
	done
	...
	for cfg_file in $prior_cfg_file ${ovpn_client_cfg_dir}/${ipvanish}/ipvanish-${1}-${2}*.ovpn
	do
